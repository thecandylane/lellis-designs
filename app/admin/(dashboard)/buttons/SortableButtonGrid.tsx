...
import { Loader2, GripVertical, Search, X } from 'lucide-react'
...
export default function SortableButtonGrid({ buttons, categories = [], rootCategories = [] }: Props) {
  ...
  const filteredItems = useMemo(() => {
    return items.filter(item => {
      ...
      const matchesCategory = selectedCategory === 'all' ||
        (typeof item.category === 'object' && item.category?.id === selectedCategory)
    ...
    <div className="relative">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-4 mb-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Filter buttons by name..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 bg-background"
            />
          </div>
        ...
        {hasActiveFilters && (
          <p className="text-sm text-muted-foreground mt-2">
            Showing {filteredItems.length} of {items.length} button{items.length !== 1 ? 's' : ''}
          </p>
        )}
      </div>
      ...
      {filteredItems.length === 0 ? (
        <div className="bg-card rounded-xl shadow-sm border border-border p-12 text-center">
          <p className="text-muted-foreground">No buttons match your filters.</p>
          {hasActiveFilters && (
            <button
              onClick={clearFilters}
