...
type Props = {
  ...
  customRequestFontOptions?: FormOption[]
  customRequestTextOptions?: FormOption[]
  customRequestDeliveryOptions?: FormOption[]
  customRequestDateFlexibility?: FormOption[]
  customRequestSuccessTitle?: string
  customRequestSuccessMessage?: string
}

export default function CustomRequestForm({
  backgroundImageUrl,
  customRequestFontOptions = [
  ...
  customRequestTextOptions = [
    { value: 'no', label: 'No text needed' },
    { value: 'yes', label: 'Yes, I have text' },
    { value: 'help', label: 'Help me decide' },
  ],
  customRequestDeliveryOptions = [
    { value: 'pickup', label: 'Local Pickup' },
    { value: 'ship', label: 'Ship to Me' },
    { value: 'either', label: 'Either Works' },
  ],
  customRequestDateFlexibility = [
    { value: 'flexible', label: 'Very flexible' },
    { value: 'somewhat', label: 'Within a week' },
    { value: 'firm', label: 'Hard deadline' },
  ],
  customRequestSuccessTitle = 'Request Submitted!',
  customRequestSuccessMessage = 'Thank you for your custom button request! We\'ll review your details and get back to you within 24-48 hours with a quote.',
}: Props) {
  ...
  if (submitted) {
    return (
      <div className={`min-h-screen flex items-center justify-center p-4 ${!hasBackgroundImage ? 'bg-glow' : ''}`}>
        {hasBackgroundImage && (
          <>
            <div className="fixed inset-0 -z-20">
              <Image src={backgroundImageUrl} alt="" fill className="object-cover" sizes="100vw" />
            </div>
            <div className="fixed inset-0 -z-10" style={{ backgroundColor: 'rgba(255, 255, 255, 0.7)' }} />
          </>
        )}
        <div className="max-w-md w-full bg-card rounded-2xl shadow-lg p-8 text-center border border-border">
          ...
          <h1 className="text-2xl font-semibold text-foreground mb-3">{customRequestSuccessTitle}</h1>
          <p className="text-muted-foreground mb-6">
            {customRequestSuccessMessage}
          </p>
  ...
  return (
    <div className={`min-h-screen ${!hasBackgroundImage ? 'bg-glow' : ''}`}>
      {/* Background Image */}
      {hasBackgroundImage && (
        <>
          <div className="fixed inset-0 -z-20">
            <Image src={backgroundImageUrl} alt="" fill className="object-cover" priority sizes="100vw" />
          </div>
          <div className="fixed inset-0 -z-10" style={{ backgroundColor: 'rgba(255, 255, 255, 0.7)' }} />
        </>
      )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {customRequestTextOptions.map((option) => (
                    <label
                      key={option.value}
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {customRequestFontOptions.map((option) => (
                        <label
                          key={option.value}
          ...
          <section className="bg-card rounded-xl shadow-md border border-border p-5">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {customRequestDateFlexibility.map((option) => (
                    <label
                      key={option.value}
                      className={`flex items-center justify-center p-3 border rounded-lg cursor-pointer transition-all ${
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {customRequestDeliveryOptions.map((option) => (
                    <label
                      key={option.value}
                      className={`flex items-center justify-center p-3 border rounded-lg cursor-pointer transition-all ${
